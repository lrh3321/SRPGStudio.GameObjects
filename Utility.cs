...
namespace SRPGStudio.GameObjects
{
	...
	public static class Utility
	{
		...
		public static void UpdateDict(IEnumerable<Class> clss)
		{
			int iname = 1;
		...
		#region "Serialize"
		static XmlSerializer ccSerializer;
		
		static void EnsureClassCollectionSerializer(){
			if (ccSerializer == null) {
				ccSerializer= new XmlSerializer(typeof(ClassCollection));
			}
		}
		
		public static ClassCollection LoadScript(Stream stream){
			EnsureClassCollectionSerializer();
			return (ClassCollection)ccSerializer.Deserialize(stream);
		}
		
		public static ClassCollection LoadScript(string str){
			EnsureClassCollectionSerializer();
			return (ClassCollection)ccSerializer.Deserialize(new StringReader(str));
		}
